...
export const FrontendNodeGateway = {
  ...
  searchNodes: async (
    ...
    try {
      return await post<IServiceResponse<INode[]>>(
        baseEndpoint + servicePath + 'searchNodes',
        {
          query: query,
        }
      );
    } catch (exception) {
      return failureServiceResponse("[updateNode] Unable to search queries");
    }
  },

  getOrganizedData: async (
  ): Promise<IServiceResponse<organizedData>> => {
    try {
      return await get<IServiceResponse<organizedData>>(
        baseEndpoint + servicePath + 'organizedData'
      );
    } catch (exception) {
      return failureServiceResponse("[updateNode] Unable to search queries");
    }
