...
export class BackendNodeGateway {
  ...
  /**
   * Method to search nodes by query string
   */
  async searchNodesByQuery(query: string): Promise<IServiceResponse<INode[]>> {
    return await this.nodeCollectionConnection.searchNodes(query);
  }

  // **********************************************************************************
  /**
   * Method to get all restaurant nodes and nodeIdToNodeMap.
   * @returns IServiceResponse<organizedData>
   */
  async getAllRestaurants(): Promise<IServiceResponse<IRestaurantNode[]>> {
    const findAllRestaurantsResp = await this.nodeCollectionConnection.findAllRestaurants();
    if (!findAllRestaurantsResp.success) {
      return failureServiceResponse(findAllRestaurantsResp.message);
    }

    return successfulServiceResponse(findAllRestaurantsResp.payload);
  }
   ...
   * @returns IServiceResponse<organizedData>
   */
  async getOrganizedData(): Promise<IServiceResponse<organizedData>> {
    const findAllRestaurantsResp = await this.nodeCollectionConnection.findAllNodes();
    if (!findAllRestaurantsResp.success) {
      return failureServiceResponse(findAllRestaurantsResp.message);
    }

    const restaurants: IRestaurantNode[] = [];
    const map: NodeIdsToNodesMap = {};
    findAllRestaurantsResp.payload.forEach((node) => {
      if (isIRestaurantNode(node)) {
        restaurants.push(node as IRestaurantNode);
      } else {
