...
export class NodeRouter {
  ...
  constructor(mongoClient: MongoClient) {
    ...
    /** 
     * Request to search nodes with query string
     */
    NodeExpressRouter.post("/searchNodes", async(req: Request, res: Response) => {
      try {
    ...
    NodeExpressRouter.get("/roots", async (req: Request, res: Response) => {
     ...
     /**
     * Request to retrieve all restaurant nodes
     *
     * @param req request object coming from client
     * @param res response object to send to client
     */
    // NodeExpressRouter.get("/allRestaurants", async (req: Request, res: Response) => {
    //   try {
    //     const response: IServiceResponse<RecursiveNodeTree[]> =
    //       await this.BackendNodeGateway.getAllRestaurants();
    //       res.status(200).send(response);
    //   } catch (e) {
    //     res.status(500).send(e.message);
    //   }
    // });
    NodeExpressRouter.get("/organizedData", async (req: Request, res: Response) => {
      try {
        const response: IServiceResponse<organizedData> =
          await this.BackendNodeGateway.getOrganizedData();
          res.status(200).send(response);
      } catch (e) {
        res.status(500).send(e.message);
      }
